...
def run():
    ...
    adults_old = list(filter(lambda worker : worker['age'] > 18, DATA))
    
    adults = list(map(lambda worker : worker['name'], adults_old))
    
    add_old = list(map(lambda worker: worker | {"old":worker["age"] > 70 }, adults_old))
    print(add_old)
